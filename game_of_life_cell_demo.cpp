...
int main () {
	...
	sim->AddComponent(new GameOfLife_Cell(sim, "test_cell", monitor_on, {{"not_clear_cycle", true}}));
	...
	for (int sibling_count_index = 0; sibling_count_index < 9; sibling_count_index ++) {
		...
		sim->Run(36, true, verbose, print_probe_samples, false);
	}
	
	//~// Next we run through all the sibling permutations for the 'live' starting state.
	//~for (int sibling_count_index = 0; sibling_count_index < 9; sibling_count_index ++) {
		//~for (int k = 0; k < 8; k ++) {
			//~std::cout << sibling_states[sibling_count_index][k];
		//~}
		...
		//~for (int pin_index = 0; pin_index < 8; pin_index ++) {
			...
			//~sim->ChildSet("test_cell", sibling_input_name, sibling_states[sibling_count_index][pin_index]);
		//~}
		//~// Now run for 16 ticks to integrate the sibling inputs and get the new Cell state.
