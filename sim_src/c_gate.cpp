...
Gate::Gate(Device* parent_device_pointer, std::string const& gate_name, std::string const& gate_type, std::vector<std::string> in_pin_names, bool monitor_on) {
	...
	m_out_pins[m_out_pin_name_hash] = new_out_pin;
}
...
void Gate::Initialise() {
	...
	m_out_pins[m_out_pin_name_hash].state = new_state;
	m_out_pins[m_out_pin_name_hash].state_changed = true;
...
void Gate::Evaluate() {
	pin* this_pin = &m_out_pins[m_out_pin_name_hash];
...
void Gate::Propagate() {
	...
	if (this_pin->state_changed) {
		...
		for (const auto& connection: m_connections) {
			...
			connection.target_component->Set(connection.target_pin_name_hash, connection.target_pin_direction, this_pin->state);
		}
	}
}
...
void Gate::PrintPinStates(int max_levels) {
	...
	std::cout << "] [ " << BoolToChar(m_out_pins[m_out_pin_name_hash].state) << " ]" << std::endl;
}
