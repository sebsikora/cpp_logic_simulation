...
void SimpleTerminal_MagicEngine::UpdateMagic() {
	...
	while (read(m_fifo_dat_slave_to_master, &data_in_buffer, 1) > 0) {
		if (((m_parent_device_pointer->m_monitor_on) || (m_parent_device_pointer->mg_verbose_output_flag)) && !chars_read_in_flag) {
			std::cout << std::endl;
		...
		if ((m_parent_device_pointer->m_monitor_on) || (m_parent_device_pointer->mg_verbose_output_flag)) {
